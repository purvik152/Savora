...
const ParseIngredientsInputSchema = z.object({
  ...
  language: z.string().describe('The language of the ingredients (e.g., "English", "Spanish").'),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an expert grocery shopping assistant. Your task is to convert a list of recipe ingredients into a clean, searchable list of grocery items suitable for an online grocery store. The ingredients are in {{language}}.

**Instructions:**
- For each ingredient in the input list, extract the core product name in its original language ({{language}}).
