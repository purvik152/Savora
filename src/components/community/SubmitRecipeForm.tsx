...
import { Loader2, Upload, Mic, Circle, WandSparkles, Image as ImageIcon } from 'lucide-react';
...
export function SubmitRecipeForm() {
  ...
  const handleGenerateImage = useCallback(async () => {
    if (!currentTitle) {
        toast({
            variant: 'destructive',
            title: "Can't Generate Image Yet",
            description: "Please enter a title for your recipe first."
        });
        return;
    }
    ...
    try {
        const result = await generateRecipeImage({ title: currentTitle });
        if (result.imageDataUri) {
            setGeneratedImageDataUri(result.imageDataUri);
            toast({ title: "AI Image Generated!", description: "You can now submit your recipe." });
        } else {
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    if (!imageFile && !generatedImageDataUri) {
        form.setError('image', { type: 'manual', message: 'Please upload an image or generate one with AI.' });
        return;
    }
    
    setLoading(true);
    
    const processAndSubmit = (imageDataUri: string) => {
        try {
    ...
    if (generatedImageDataUri) {
        processAndSubmit(generatedImageDataUri);
    } else if (imageFile) {
        const reader = new FileReader();
        reader.onload = (e) => {
            const imageDataUri = e.target?.result as string;
            processAndSubmit(imageDataUri);
        };
        reader.onerror = () => {
            toast({
                variant: 'destructive',
        ...
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Button type="button" variant="outline" onClick={handleSuggestTitles} disabled={isSuggesting || isGeneratingImage}>
                    ...
                    <FormItem>
                        ...
                        <Card className="p-4 bg-secondary/50">
                            <div className="flex flex-col sm:flex-row items-center gap-4">
                                <FormControl>
                                    <Input type="file" accept="image/*" {...fileRef} className="flex-grow"/>
                                </FormControl>
                                <span className="text-muted-foreground font-bold">OR</span>
                                <Button type="button" onClick={handleGenerateImage} disabled={isGeneratingImage || isSuggesting}>
                                    {isGeneratingImage ? <Loader2 className="h-4 w-4 animate-spin"/> : <ImageIcon className="h-4 w-4" />}
                                    <span className="ml-2">Generate AI Image</span>
                                </Button>
                            </div>
                             {generatedImageDataUri && (
                                <div className="mt-4 relative w-full aspect-video rounded-lg overflow-hidden border">
                                    <Image src={generatedImageDataUri} alt="AI generated recipe image" fill className="object-cover"/>
                                </div>
                            )}
                        </Card>
                        <FormDescription>Upload your own image, or let our AI create one for you based on your title.</FormDescription>
                        <FormMessage />
                    </FormItem>
                )}
              />
              ...
              <Button type="submit" disabled={loading || isGeneratingImage} className="w-full">
                {loading ? (
